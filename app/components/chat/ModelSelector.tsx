...
export const ModelSelector = ({
  ...
  const filteredModels = useMemo(() => {
    ...
    return baseModels
      ...
      .sort((a, b) => {
        // Sort by search score (highest first) when searching
        if (debouncedModelSearchQuery) {
          return b.searchScore - a.searchScore;
        }

        // Preserve curated provider order (OpenRouter top models stay first)
        return 0;
      });
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1 ml-2">
                        {isModelLikelyFree(modelOption, provider?.name) && (
                          <span className="i-ph:gift text-xs text-cyan-400" title="Free model" />
                        )}
